...
const updateTeamStmt = db.prepare(`
  UPDATE teams
  SET name = ?, notes = ?, shield_url = ?, updated_at = ?
  WHERE id = ?
`);
...
async function handleApi(req, res, url) {
  ...
  if (req.method === 'PUT' && /^\/api\/teams\/\d+$/.test(url.pathname)) {
    ...
    const name = cleanText(body.name, 80);
    const notes = cleanText(body.notes, 240);
    const shieldUrl = cleanText(body.shieldUrl, 255);
    ...
    const result = updateTeamStmt.run(name, notes, shieldUrl || null, updatedAt, teamId);
...
function computeStandings(phaseTournamentId, groupName = null) {
  ...
  if (groupName) {
    const rows = db.prepare(`
      SELECT t.id, t.slot_number AS slotNumber, t.name, t.shield_url AS shieldUrl
    ...
    teams = db.prepare('SELECT id, slot_number AS slotNumber, name, shield_url AS shieldUrl FROM teams ORDER BY slot_number ASC').all().map(mapPlainObject);
  }
  ...
  for (const t of teams) {
    stats[t.id] = { id: t.id, slotNumber: t.slotNumber, name: t.name, shieldUrl: t.shieldUrl, played:0, won:0, draw:0, lost:0, goalsFor:0, goalsAgainst:0 };
  }
  ...
  for (const m of playedMatches) {
    ...
    if (m.homeGoals > m.awayGoals)      { home.won++; away.lost++; }
    else if (m.homeGoals < m.awayGoals) { away.won++; home.lost++; }
    else                                { home.draw++; away.draw++; }
  }

  return Object.values(stats)
    .map(s => ({ ...s, goalDiff: s.goalsFor - s.goalsAgainst, points: s.won * 3 + s.draw }))
...
async function handleFixtureApi(req, res, url) {
  ...
  if (req.method === 'GET' && standingsMatch) {
    ...
    sendJson(res, 200, computeStandings(tournamentId, group));
    return;
  }
  ...
  if (req.method === 'POST' && cardsReportMatch) {
    ...
    const ins = db.prepare('INSERT INTO match_cards (match_id, player_id, team_id, card_type, minute, suspension_matches, is_pending_suspension, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)').run(
      matchId, playerId, teamId, cardType, minute, suspensionMatches, isPending ? 1 : 0, nowIso()
    );
...
async function handleStatsApi(req, res, url) {
  const tournamentId = Number(url.searchParams.get('tournamentId') || 1);
  const group = url.searchParams.get('group') || null;
  ...
  if (req.method === 'GET' && url.pathname === '/api/stats/scorers') {
    const scorers = db.prepare(`
      ...
      WHERE m.phase_tournament_id = ? AND g.is_own_goal = 0
      GROUP BY p.id
  ...
  if (req.method === 'GET' && url.pathname === '/api/stats/suspended') {
    try {
      const maxRoundRow = db.prepare(`SELECT MAX(round_number) as maxRound FROM matches WHERE status='played' AND phase_tournament_id = ?`).get(tournamentId);
      ...
      const suspended = db.prepare(`
        SELECT mc.id AS cardId, p.id, p.name AS playerName, t.name AS teamName, t.shield_url AS shieldUrl,
               ...
               (
                 ...
                 WHERE m2.status='played' 
                   AND m2.phase_tournament_id = m.phase_tournament_id
                   AND (m2.home_team_id = mc.team_id OR m2.away_team_id = mc.team_id)
        ...
        JOIN teams t ON t.id = mc.team_id
        JOIN matches m ON m.id = mc.match_id
        WHERE m.phase_tournament_id = ? AND mc.card_type = 'red'
          AND (
        ...
        ORDER BY isPending DESC, remainingMatches DESC, p.name ASC
      `).all(tournamentId).map(mapPlainObject);
...
function getMatchCardsReport(matchId) {
  ...
  const matchCards = db.prepare(`
    SELECT id, player_id AS playerId, team_id AS teamId, card_type AS cardType, minute, suspension_matches AS suspensionMatches FROM match_cards WHERE match_id = ?
  `).all(matchId).map(mapPlainObject);
