...
function cacheElements() {
  [
    ...
    'deleteModalTitle', 'deleteModalDesc', 'exportPdfBtn',
    'cancelEditBtn', 'submitBtnText', 'teamFilterSelect', 'movementsEditToggle',
    // Stats & Report
    ...
    'btnCancelReport', 'btnGenerateReportPdf',
    // Edit Player Modal
    ...
    card.innerHTML = `
      <div class="flex items-center gap-4 mb-4">
        ...
        <div class="min-w-0 flex-1">
          <p class="text-sm font-bold text-white truncate">${escapeHTML(team.name || 'Sin nombre')}</p>
...
function renderMovements() {
  ...
  if (state.currentFilter === 'teams-insc') {
    ...
    els.movementsTbody.innerHTML = sortedTeams.map(team => {
      const pct = targetAmountCents > 0 ? Math.round((team.total / targetAmountCents) * 100) : 0;
...
/* ── FIXTURE STATS TAB LOGIC ADDENDUM (BUG FIX) ── */
// Ensure standings calculation functions use standard and abbreviated properties properly
...
async function renderFixtureContent() {
  ...
  // Disable T2 sub-buttons until T2 is generated
  const hasTournament = fixture.t1Id || fixture.t2Id;
  if (!hasTournament) {
    content.innerHTML = '<p class="text-center text-gray-600 py-8">No hay fases disponibles aún.</p>';
    return;
  }

  content.innerHTML = '<div class="text-center text-gray-600 py-8 animate-pulse">Cargando...</div>';

  try {
    const { sub } = { sub: fixture.currentSub };

    if (sub === 't1') {
      if (!fixture.t1Id) { content.innerHTML = '<p class="text-gray-500 text-center py-6">Torneo 1 no disponible.</p>'; return; }
      const [standings, rounds] = await Promise.all([
        fetchJSON(`/api/fixture/tournaments/${fixture.t1Id}/standings`),
        fetchJSON(`/api/fixture/tournaments/${fixture.t1Id}/matches`),
      ]);
      content.innerHTML = renderStandingsHtml(standings, true) + renderGenerateT2Btn(standings) + renderRoundsHtml(rounds);
    } else if (sub === 't2a' || sub === 't2b') {
      if (!fixture.t2Id) { content.innerHTML = renderT2EmptyState(); return; }
      const group = sub === 't2a' ? 'A' : 'B';
      const [standings, rounds] = await Promise.all([
        fetchJSON(`/api/fixture/tournaments/${fixture.t2Id}/standings?group=${group}`),
        fetchJSON(`/api/fixture/tournaments/${fixture.t2Id}/matches?group=${group}`),
      ]);
      if (!standings.length) { content.innerHTML = renderT2EmptyState(); return; }
      content.innerHTML = renderStandingsHtml(standings, false) + renderRoundsHtml(rounds);
    }
  } catch (e) {
    console.error(e);
    content.innerHTML = '<p class="text-red-400 text-center py-6">Error al cargar los datos del fixture.</p>';
  }
}
...
function renderStandingsHtml(standings, showZoneBadge) {
  ...
  return `
    <section class="bg-surface-850 rounded-2xl border border-white/5 overflow-hidden mb-4">
...
async function renderFixtureContent() {
  ...
  if (!tournamentId) {
    container.innerHTML = '<p class="text-center py-10 text-gray-500">Torneo no encontrado.</p>';
    return;
  }

  try {
    const query = sub.startsWith('t2') ? `?group=${sub === 't2a' ? 'A' : 'B'}` : '';
    const [rounds, allTeams] = await Promise.all([
      fetchJSON(`/api/fixture/tournaments/${tournamentId}/matches${query}`),
      fetchJSON('/api/teams')
    ]);

    // Store teams for selectors
    fixture.teams = allTeams;

    // For T2, we want to only show teams that are actually in that group
    let groupTeams = allTeams;
    if (sub.startsWith('t2')) {
      const teamIds = new Set();
      rounds.forEach(r => r.matches.forEach(m => {
        teamIds.add(m.homeTeamId);
        if (m.awayTeamId) teamIds.add(m.awayTeamId);
      }));
      groupTeams = allTeams.filter(t => teamIds.has(t.id));
    }

    container.innerHTML = renderRoundsHtml(rounds, groupTeams, tournamentId);
  } catch (err) {
    console.error(err);
...
window.addMatchCard = async function (matchId, playerId, teamId, cardType) {
  ...
  try {
    await fetchJSON(`/api/fixture/matches/${matchId}/cards`, {
      method: 'POST',
      body: JSON.stringify({ playerId, teamId, cardType, suspensionMatches, isPending }),
    });
...
async function openStatsReportModal() {
  ...
  // Find max round from current fixture state
  let maxRound = 15;
  if (window.fixture && window.fixture.phases && window.fixture.phases.length) {
    // Attempt to find the max round number from loaded matches
    // For now, let's keep it simple or look at the fixture data if loaded
    maxRound = 15; // Standard for this league
  }
...
async function generateStatsReport() {
  ...
  try {
    const tournamentId = 1; // Default Apertura
    const [standingsRes, scorersRes, cardsRes, suspendedRes, detailedResultsRes] = await Promise.all([
      fetch('/api/stats/standings'),
      fetch('/api/stats/scorers'),
      fetch('/api/stats/cards'),
      fetch('/api/stats/suspended'),
                ...
                ` : suspended.map(s => `
                  <tr>
                    ...
                    <td class="text-center">
                      <div class="text-xs font-bold text-gray-300">Roja Directa</div>
...
async function renderEstadisticas() {
  ...
  try {
    ...
    if (els.statsResultsRoundSelect && els.statsResultsRoundSelect.options.length === 0) {
      // Find max round from fixture if possible or default to 15
      const rounds = 15; // Assume 15 for now or fetch
      ...
      try {
        const resList = await fetch('/api/stats/current-round');
    ...
    const [standingsRes, scorersRes, cardsRes, suspendedRes] = await Promise.all([
      fetch('/api/stats/standings'),
      fetch('/api/stats/scorers'),
      fetch('/api/stats/cards'),
      fetch('/api/stats/suspended')
      ...
      <tr class="hover:bg-white/5 transition-colors border-b border-white/5 group">
        ...
        <td class="py-3 px-4">
          <div class="flex items-center gap-3">
            ...
            <span class="font-bold text-gray-200 text-sm">${escapeHTML(s.name)}</span>
          </div>
        </td>
        ...
        els.statsSuspendedTbody.innerHTML = suspended.map(s => {
          return `
            <tr class="hover:bg-red-500/5 transition-colors border-b border-white/5 group">
              ...
              <td class="py-3 px-4 text-center">
                <div class="text-[11px] font-bold text-gray-300">Roja Directa</div>
                <div class="text-[10px] text-gray-500 uppercase font-semibold mt-0.5">
...
async function renderResultsByDate(round) {
  ...
  try {
    const res = await fetch(`/api/stats/results?round=${round}`);
